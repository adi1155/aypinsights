...
<div class="grid gap-8 lg:grid-cols-2">
    <div class="glass-card p-6">
        ...
        <form method="POST" action="{{ route('settings.scheduled.store') }}" class="space-y-3">
            @csrf
            <select name="report_type" class="w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-white"><option value="ceo">CEO</option><option value="daily_closing">Daily Closing</option><option value="ap">AP</option><option value="ar">AR</option><option value="expense">Expense</option></select>
